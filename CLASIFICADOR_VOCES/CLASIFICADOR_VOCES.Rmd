...
Para su cálculo, se emplea la función zcr en R, utilizando ventanas de 2s con un solapamiento del 50%. Posteriormente, se realiza el promedio de los valores obtenidos en cada ventana para obtener un único valor representativo por señal, similar al enfoque utilizado para calcular la frecuencia fundamental. Esta media permite consolidar la información de toda la señal en un solo parámetro útil para el análisis.
...
Notamos un comportamiento muy discriminante: los CE de las voces masculinas se encuentran mucho más concentradas en torno a una media cercana a 220 mientras que las voces femeninas están  mayoritariamente concentradas alrededor de 250. Destacar que las voces femeninas tienen un máximo local alrededor de 220.
...
El primer paso antes de hacer ningún algoritmo de ML es añadir a nuestro dataframe de características las posibles etiquetas de estos algoritmos. En este caso las etiquetas se encuentran en el dataframe meta_audios, siendo estas el locutor y el género.
...
A continuación, empleando las características obtenidas de la selección, trataremos de clasificar utilizando los modelos sugeridos.
...
Teniendo un Accuracy de `r cm_rf$overall["Accuracy"]` para el RF y de `r cm_svm$overall["Accuracy"]` para el SVM. 
