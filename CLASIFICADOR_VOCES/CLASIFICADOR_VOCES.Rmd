...
# PS
# Inicializar el vector para la característica PS
PS_carac <- rep(0.0, length(audios))
...
for (audio in audios) {
  ...
  # Calcular PS para cada canal
  PS_iz <- PS(audio_iz, audio@samp.rate, span=NULL)
  PS_de <- PS(audio_de, audio@samp.rate, span=NULL)
...
```{r}
audio_iz <- audios[[1]]@left
audio_de <- audios[[1]]@right
