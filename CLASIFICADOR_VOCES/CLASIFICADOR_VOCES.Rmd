...
La frecuencia fundamental es una característica ampliamente utilizada en el análisis de secuencias de audio para diferenciar géneros y hablantes, ya que representa el tono base de la voz, más bajo en hombres que en mujeres. Su variabilidad entre individuos la convierte también en un indicador relevante para la identificación de locutores.

Para calcularla, se divide la señal en ventanas de 2 segundos y se realiza la Transformada de Fourier en cada una. La frecuencia fundamental se identifica como aquella con la máxima amplitud en la mitad positiva del espectro. Finalmente, se calcula una media ponderada de estas frecuencias, asignando mayor peso a las primeras ventanas y eliminando los últimos 4 segundos, que suelen ser menos relevantes debido a que el locutor generalmente ya ha terminado de hablar.
...
Para su cálculo, se emplea la función zcr en R, utilizando ventanasde 2s con un solapamiento del 50%. Posteriormente, se realiza el promedio de los valores obtenidos en cada ventana para obtener un único valor representativo por señal, similar al enfoque utilizado para calcular la frecuencia fundamental. Esta media permite consolidar la información de toda la señal en un solo parámetro útil para el análisis.
